...
export default function NavigationBarUpdateForm(props) {
  const {
    id: idProp,
    navigationBar,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = navigationBarRecord
      ? { ...initialValues, ...navigationBarRecord }
      : initialValues;
    setPhoto(cleanValues.photo);
    setAbout(cleanValues.about);
    setBlog(cleanValues.blog);
    setConnect(cleanValues.connect);
    setErrors({});
  };
  const [navigationBarRecord, setNavigationBarRecord] =
    React.useState(navigationBar);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(NavigationBar, idProp)
        : navigationBar;
      setNavigationBarRecord(record);
    };
    queryData();
  }, [idProp, navigationBar]);
  React.useEffect(resetStateValues, [navigationBarRecord]);
  const validations = {
    photo: [],
    about: [],
    blog: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            NavigationBar.copyOf(navigationBarRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "NavigationBarUpdateForm")}
      {...rest}
    >
      <TextField
        label="Photo"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || navigationBar)}
